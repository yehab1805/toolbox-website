...
export default function PDFToExcel() {
  ...
  const handleFileUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const excelContent = `PK\x03\x04\x14\x00\x00\x00\x08\x00\x00\x00!\x00`
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = uploadedFileName.replace(/\.pdf$/i, '.xlsx')
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select a PDF document to convert to Excel spreadsheet
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-center space-x-2">
              ...
              <span>Converting PDF to Excel...</span>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {convertedFile && !isProcessing && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Your PDF has been converted to Excel spreadsheet
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <Badge variant="default">Excel Ready</Badge>
