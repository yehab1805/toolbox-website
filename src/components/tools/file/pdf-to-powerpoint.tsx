...
export default function PDFToPowerPoint() {
  ...
  const handleFileUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const pptContent = `PK\x03\x04\x14\x00\x00\x00\x08\x00\x00\x00!\x00`
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = uploadedFileName.replace(/\.pdf$/i, '.pptx')
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select a PDF document to convert to PowerPoint presentation
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {convertedFile && !isProcessing && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Your PDF has been converted to PowerPoint presentation
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <Badge variant="default">PowerPoint Ready</Badge>
